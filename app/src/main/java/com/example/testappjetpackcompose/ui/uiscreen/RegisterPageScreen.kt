...
import com.example.testappjetpackcompose.model.User
...
fun RegisterScreen(
    ...
    Column(
        ...
        Button(onClick = {
            signUpUser(etName, etEmail, etPass, etConfPass, mainActivity, auth, db, navController)
        }) {
            Text(text = "Register")
        }
...
fun signUpUser(
    ...
    auth.createUserWithEmailAndPassword(email, pass).addOnCompleteListener(mainActivity) {
        if (it.isSuccessful) {
            //AddUserInDatabase
            addUserDatabase(name, email, auth.currentUser?.uid!!, db)
            navController.navigate("third")
        } else {
...
private fun addUserDatabase(name: String, email: String, uid: String, db: FirebaseDatabase) {
    db.getReference("User").child(uid).setValue(User(name, email, uid))
}
